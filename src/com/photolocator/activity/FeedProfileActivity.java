...
import com.photolocator.adapter.FeedProfileCommentsAdapter;
import com.photolocator.adapter.SimpleListDialogAdapter;
...
import com.photolocator.popupwindow.OtherFeedListPopupWindow;
import com.photolocator.popupwindow.OtherFeedListPopupWindow.onOtherFeedListPopupItemClickListner;
import com.photolocator.utility.DateUtils;
import com.photolocator.utility.JsonResolveUtils;
import com.photolocator.utility.PhotoUtils;
import com.photolocator.view.EmoteInputView;
import com.photolocator.view.EmoticonsEditText;
...
public class FeedProfileActivity extends BaseActivity implements OnItemClickListener, onRightImageButtonClickListener,
		onOtherFeedListPopupItemClickListner, OnClickListener, OnTouchListener
{
	...
	protected void initViews()
	{
		...
		mIvVip = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_vip);
		mIvGroupRole = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_group_role);
		mIvIndustry = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_industry);
		mIvWeibo = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_weibo);
		mIvTxWeibo = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_txweibo);
		mIvRenRen = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_renren);
		mIvDevice = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_device);
		mIvRelation = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_relation);
		mIvMultipic = (ImageView) mHeaderView.findViewById(R.id.user_item_iv_icon_multipic);
		mEtvContent = (EmoticonsTextView) mHeaderView.findViewById(R.id.header_feed_etv_content);
		mIvContent = (ImageView) mHeaderView.findViewById(R.id.header_feed_iv_content);
	...
	private void init()
	{
		...
		mAdapter = new FeedProfileCommentsAdapter(mApplication, FeedProfileActivity.this, mComments);
	...
	{
		mWidthAndHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 120, getResources()
				.getDisplayMetrics());
	...
	private void initHeaderView()
	{
		...
		if (mPeople.getIsGroupRole() != 0)
		{
			...
			{
				mIvGroupRole.setImageResource(R.drawable.ic_userinfo_groupowner);
			}
		}
		else
		{
		...
		if (!android.text.TextUtils.isEmpty(mPeople.getIndustry()))
		{
			...
			mIvIndustry.setImageBitmap(PhotoUtils.getIndustry(FeedProfileActivity.this, mPeople.getIndustry()));
		}
		else
		{
			mIvIndustry.setVisibility(View.GONE);
	...
	public void onClick()
	{
		mPopupWindow.showAtLocation(mHeaderLayout, Gravity.RIGHT | Gravity.TOP, -10, mHeaderHeight + 10);
	}
	...
	public void onClick(View v)
	{
		...
		case R.id.feedprofile_btn_send:
			...
			else
			{
				...
				FeedComment comment = new FeedComment("测试用户", "nearby_people_other", content, DateUtils.formatDate(
						FeedProfileActivity.this, System.currentTimeMillis()));
	...
	private class OnReportDialogItemClickListener implements onSimpleListItemClickListener
	{
	...
	private class OnReplyDialogItemClickListener implements onSimpleListItemClickListener
	{
	...
	private void showKeyBoard()
	{
		...
		((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).showSoftInput(mEetEditer, 0);
	}

	private void hideKeyBoard()
	{
		((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(FeedProfileActivity.this
