package com.photolocator.utility;
...
import com.photolocator.entity.Feed;
import com.photolocator.entity.FeedComment;
import com.photolocator.entity.NearByPeople;
import com.photolocator.entity.NearByPeopleProfile;
...
public class JsonResolveUtils
{
	...
	public static boolean resolveNearbyPeople(BaseApplication application)
	{
		...
		application.mNearByPeoples.add(people);
		people = new NearByPeople("cde", "Mr. Xu", 1, 1, "cds", 1, 1, 1, 1, 1, 1, "James McGill", 1, 1, "2013", "2014",
				"ok");
		application.mNearByPeoples.add(people);

		people = new NearByPeople("cde", "Mr. Xu", 1, 1, "cds", 1, 1, 1, 1, 1, 1, "John Smith", 1, 1, "2013", "2014",
				"ok");
		application.mNearByPeoples.add(people);

		people = new NearByPeople("cde", "Mr. Xu", 1, 1, "cds", 1, 1, 1, 1, 1, 1, "Steven. Low", 1, 1, "2013", "2014",
				"ok");
			...
			if (json != null)
			{
				try
				{
					...
					for (int i = 0; i < array.length(); i++)
					{
						...
						int isGroupRole = object.getInt(NearByPeople.GROUP_ROLE);
						String industry = object.getString(NearByPeople.INDUSTRY);
						int isbindWeibo = object.getInt(NearByPeople.WEIBO);
						int isbindTxWeibo = object.getInt(NearByPeople.TX_WEIBO);
						...
						String distance = object.getString(NearByPeople.DISTANCE);
	 ...
	 */
	/*
	 * public static boolean resolveNearbyGroup(BaseApplication application) { if (application.mNearByGroups != null &&
	 * application.mNearByGroups.isEmpty()) { String json = TextUtils.getJson(application.getApplicationContext(),
	 * NEARBY_GROUP); if (json != null) { try { JSONArray array = new JSONArray(json);
	 * 
	 * NearByGroup group = null; JSONObject object = null; JSONArray groupsArray = null; List<NearByGroups> groupsList =
	 * null; NearByGroups groups = null;
	 * 
	 * for (int i = 0; i < array.length(); i++) { object = array.getJSONObject(i); String address =
	 * object.getString(NearByGroup.ADDRESS); String distance = object.getString(NearByGroup.DISTANCE); int groupCount =
	 * object.getInt(NearByGroup.GROUP_COUNT);
	 * 
	 * groupsArray = object.getJSONArray(NearByGroup.GROUPS); groupsList = new ArrayList<NearByGroups>(); for (int j =
	 * 0; j < groupsArray.length(); j++) { JSONObject o = groupsArray.getJSONObject(j); String avatar =
	 * o.getString(NearByGroups.AVATAR); String name = o.getString(NearByGroups.NAME); int isNew =
	 * o.getInt(NearByGroups.IS_NEW); int isParty = o.getInt(NearByGroups.IS_PARTY); int memberCount =
	 * o.getInt(NearByGroups.MEMBER_COUNT); int memberTotal = o.getInt(NearByGroups.MEMBER_TOTAL); int isVip =
	 * o.getInt(NearByGroups.IS_VIP); int level = o.getInt(NearByGroups.LEVEL); String sign =
	 * o.getString(NearByGroups.SIGN); groups = new NearByGroups(avatar, name, isNew, isParty, memberCount, memberTotal,
	 * isVip, level, sign); groupsList.add(groups); } group = new NearByGroup(address, distance, groupCount,
	 * groupsList); application.mNearByGroups.add(group); } } catch (JSONException e) {
	 * application.mNearByGroups.clear(); } } } if (application.mNearByGroups.isEmpty()) { return false; } else { return
	 * true; }
	 */
	// }
		...
		if (!android.text.TextUtils.isEmpty(uid))
		{
			...
			if (json != null)
			{
				try
				{
					...
					String avatar = object.getString(NearByPeopleProfile.AVATAR);
					...
					String constellation = object.getString(NearByPeopleProfile.CONSTELLATION);
					String distance = object.getString(NearByPeopleProfile.DISTANCE);
					...
					{
						isHasSign = true;
						JSONObject signObject = object.getJSONObject(NearByPeopleProfile.SIGNATURE);
						sign = signObject.getString(NearByPeopleProfile.SIGN);
						if (signObject.has(NearByPeopleProfile.SIGN_PIC))
						{
							signPic = signObject.getString(NearByPeopleProfile.SIGN_PIC);
						}
						signDis = signObject.getString(NearByPeopleProfile.SIGN_DIS);
					}

					JSONArray photosArray = object.getJSONArray(NearByPeopleProfile.PHOTOS);
	 ...
	 * @return
	 */
	public static boolean resoleFeedComment(Context context, List<FeedComment> comments)
	{
