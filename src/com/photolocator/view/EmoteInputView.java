...
import com.photolocator.adapter.EmoteAdapter;

public class EmoteInputView extends LinearLayout implements OnClickListener, OnCheckedChangeListener,
		OnItemClickListener
{
		...
		case R.id.emotionbar_iv_delete:
			if (mEEtView != null)
			{
				...
				if ("]".equals(lastContent) && last > lastChar)
				{
					if (last != -1)
					{
						mEEtView.setText(startContent.substring(0, last) + endContent);
						// 定位光标位置
				...
				mEEtView.setText(startContent.substring(0, start - 1) + endContent);
				// 定位光标位置
				CharSequence info = mEEtView.getText();
				if (info instanceof Spannable)
				{
